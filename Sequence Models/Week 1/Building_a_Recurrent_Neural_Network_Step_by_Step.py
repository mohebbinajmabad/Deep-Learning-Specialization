...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    a_prev_xt = np.transpose(np.concatenate((a_prev, xt), axis=0))
    dWf = np.dot(dft, a_prev_xt)
    dWi = np.dot(dit, a_prev_xt)
    dWc = np.dot(dcct, a_prev_xt)
    dWo = np.dot(dot, a_prev_xt)
